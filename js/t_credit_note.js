$(document).ready(function(){
  ...
  if(e.keyCode==46){
   ...
   $("#btnStatuss").click(function() {
    if($("#hid").val()!=0){
      if(confirm("Are You Sure To Approve (Change Status) Cerdit Note?")){
        status_update();
      }else{
        location.href='';
      }
    }else{
      set_msg("please load credit note before approve");
    }
  });
...
function status_update(){
  $.post("index.php/main/load_data/t_credit_note/status_updation", {
    nno:$("#hid").val()
  }, function(r){
    if(r==1){

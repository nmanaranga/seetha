$(document).ready(function(){
    ...
    $("#btnReset").click(function(){
        location.href="?action=m_item_commission";
    });
...
function get_data_table(){
    $.post("index.php/main/load_data/m_item_commission/get_data_table", {
...
function check_code(){
    loding();
    var nno = $("#nno").val();
    $.post("index.php/main/load_data/m_item_commission/check_code", {
        nno : nno
    }, function(res){
        if(res == 1){
...
function set_delete(nno){
    if(confirm("Are you sure delete transaction no "+nno+"?")){
        loding();
        $.post("index.php/main/delete/m_item_commission", {
            nno : nno
        }, function(res){
            if(res == 1){
                loding();
...
function set_edit(nno){
    loding();
    $.post("index.php/main/get_data/m_item_commission", {
        nno : nno
    }, function(res){
       ...
       for(var i=0; i<res.c.length; i++){
        ...
        $("#1_"+i).val(res.c[i].rate);
        $("#2_"+i).val(res.c[i].amount);
    }  

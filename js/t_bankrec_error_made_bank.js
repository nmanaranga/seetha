$(document).ready(function(){
  ...
  if(rec_no != undefined){
	...
	$("#btnReset").click(function(){
		location.href="index.php?action=t_bankrec_error_made_bank";
	});
...
function total(){
  ...
  $(".amo").each(function(e){
    if($("#2_"+e).val()!="" && $("#4_"+e).val()!=""){
      tot_dr+=parseFloat($("#4_"+e).val());
    }
    if($("#2_"+e).val()!="" && $("#5_"+e).val()!=""){
      tot_cr+=parseFloat($("#5_"+e).val());
    }
  });
  $("#net_dr").val(m_round(tot_dr));
  $("#net_cr").val(m_round(tot_cr));
}
...
function set_delete(code){
    if(confirm("Are you sure delete no "+code+"?")){
        loding();
        $.post("index.php/main/delete/t_bankrec_error_made_bank", {
            code : code
        }, function(res){
            if(res == 1){
                delete_msg();
...
function chk_is_update(code){
  loding();
  $.post("index.php/main/load_data/t_bankrec_error_made_bank/chk_is_update", {
      code : code
  }, function(res){
      if(res == 1){
          load_data(code);
...
function load_data(code){
    loding();
    $.post("index.php/main/get_data/t_bankrec_error_made_bank", {
        code : code
    }, function(res){
        for(var x=0; x<res.det.length; x++){
          ...
          $("#5_"+x).val(res.det[x].cr_amount);
          $("#6_"+x).val(res.det[x].account);
          $("#7_"+x).val(res.det[x].account_des);
        }

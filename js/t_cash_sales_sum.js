...
var t_code = window.location.search.split('=')[2];
var p_code = window.location.search.split('=')[3];
...
$(function(){
    ...
    $("#btnHold").click(function(){
        if(confirm("Are You Sure Want to Hold Bill ?")){
            $("#approve_status").val("0");
            $("#bill_hold_status").val("1");
            $("#save_status").val("1");
            save();
        }
    });
...
function settings2(){
    $("#item_list2 .cl").click(function(){
        if($(this).children().eq(0).html() != "&nbsp;"){
            if(check_item_exist4($(this).children().eq(0).html())){
...
function load_hold_data(id){
    var g=[];

    empty_grid();
    loding();
    $.post("index.php/main/load_data/t_cash_sales_sum/load_hold_data", {
        id: id
    }, function(r){
        var fre =parseFloat(0);
        if(r=="2"){

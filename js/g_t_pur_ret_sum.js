...
$(document).ready(function(){
  ...
  $("#tgrid").tableScroll({height:200, width:990});

  $("#tgrid1").tableScroll({height:200, width:990});
  ...
  $("#btnReset").click(function(){    
   location.href="?action=g_t_pur_ret_sum";   
 });
  ...
  $("#grnno").keydown(function(e){
    if(e.keyCode == 13){
      empty_grid2();
      empty_grid()

      $.post("index.php/main/load_data/g_t_pur_ret_sum/get_invoice", {
        code:$("#grnno").val()
      },function(r){
       if(r.det==2){
        set_msg("No records");
...
function settings1(){
  $("#tgrid .cl1").dblclick(function(){        
    if($(this).children().eq(0).html() != "&nbsp;"){
     ...
     var item_code=$(this).children().find('input').eq(0).val();
     var item_des=$(this).children().find('input').eq(2).val();
     var return_qty=$(this).children().find('input').eq(3).val();
     var qty=$(this).children().find('input').eq(4).val();
     var price=$(this).children().find('input').eq(5).val();

     var bal = parseFloat(qty)-parseFloat(return_qty);
     ...
     for(var i=0; i<25 ;i++){
      if(item_code == "0" || item_code ==""){
        ...
        if($("#0_"+i).val()==item_code){
          ...
          if(parseInt(parseInt(return_qty))>=parseInt(qty)){
            set_msg("This Item ("+item_code+") Maximum return qty is "+ bal);
          }else{
            if($("#df_is_serial").val()=='1'){
...
function validate(){
  ...
  var cont=parseInt(0);
  for(var a=0; a<25 ;a++){
    if($("#01_"+i).val()==$("#0_"+a).val()){
      if($("#2_"+a).val()!=''){
        cont=cont+parseInt($("#2_"+a).val());
        $("#maxretqty_"+a).val(cont);          
        if(cont>parseInt($("#rmax_"+a).val())){
          set_msg("This item "+$("#0_"+a).val()+" maximum return qty is "+$("#rmax_"+a).val());
          return false;
        }
      }
    }
...
if($("#id").val().length==0 || $("#id").val().length<0){
 ...
 set_msg("Please enter return reason","error");
  ...
  if(grn_no!=0){
    $.post("index.php/main/load_data/g_t_pur_ret_sum/check_grn_no/",{
...
function check_grn_result(res){   
  if(res==1){  
    ...
    set_msg("Please check the grn no with supplier");
    return false;
  }
}
...
function set_delete(){
  ...
  if(id != 0){
    if(confirm("Are you sure to delete this purchase return ["+$("#hid").val()+"]? ")){
      $.post("index.php/main/delete/g_t_pur_ret_sum", {
        trans_no:$("#hid").val(),
      },function(r){
        if(r != 1){
          set_msg(r);
...
function load_data(id){
  ...
  empty_grid2();
  loding();
  $.post("index.php/main/get_data/g_t_pur_ret_sum/", {
    id: id
  }, function(r){
    if(r=="2"){
     set_msg("No records");

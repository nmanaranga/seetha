...
$this->pdf->SetFont('helvetica', 'B', 16);
		...
		foreach($det as $row){
			$from=$row->store_from;
			$from_name=$row->store_fr_name;
			$to=$row->store_to;
			$to_name=$row->store_to_name;
			$date=$row->action_date;
			$officer=$row->name;
			$ref_no=$row->ref_no;
			$nno=$row->nno;
			$group_sale_id=$row->group_sale_id;
			$group_sale_nm=$row->gr_name;
		}
		...
		$this->pdf->Cell(90, 1, $from. " - ".$from_name, '0', 0, 'L', 0);
		...
		$this->pdf->Cell(90, 1, $to." - ".$to_name, '0', 0, 'L', 0);
		...
		$this->pdf->Cell(90, 1, $group_sale_id." - ".$group_sale_nm, '0', 0, 'L', 0);
		...
		$this->pdf->Cell(15, 6,"Batch No", '1', 0, 'C', 0);
		...
		foreach($det as $row){
			...
			$this->pdf->Cell(15, 6,$row->dqty, '1', 0, 'R', 0);
			$this->pdf->Cell(18, 6,$row->min, '1', 0, 'R', 0);
			$this->pdf->Cell(18, 6,$row->max, '1', 0, 'R', 0);
			$this->pdf->Ln();

...
$this->pdf->SetFont('helvetica', 'B', 16);
		...
		if($duplicate=="1"){
			...
			$this->pdf->Cell(0, 5,'VOUCHER (Duplicate) ',0,false, 'C', 0, '', 0, false, 'M', 'M');			
		}
		...
		$this->pdf->MultiCell(20, 5,"Cluster",  1, 'C', 0, 0, '', '', true, 0, false, true, 0);
		$this->pdf->MultiCell(50, 5,"Branch",  1, 'C', 0, 0, '', '', true, 0, false, true, 0);
		$this->pdf->MultiCell(30, 5,"Trans Type",  1, 'C', 0, 0, '', '', true, 0, false, true, 0);
		$this->pdf->MultiCell(15, 5,"No",  1, 'C', 0, 0, '', '', true, 0, false, true, 0);
		$this->pdf->MultiCell(20, 5,"Date",  1, 'C', 0, 0, '', '', true, 0, false, true, 0);
		$this->pdf->MultiCell(30, 5,"Amount",  1, 'C', 0, 0, '', '', true, 0, false, true, 0);
		$this->pdf->MultiCell(30, 5,"Payment", 1, 'C', 0, 1, '', '', true, 0, false, true, 0);
		...
		foreach($grid as $row){
			...
			$this->pdf->MultiCell(20, $heigh,$row->sub_cl,  1, 'L', 0, 0, '', '', true, 0, false, true, 0);
			$this->pdf->MultiCell(50, $heigh,ucfirst(strtolower($row->mb_name))." - ".$row->sub_bc,  1, 'L', 0, 0, '', '', true, 0, false, true, 0);
			$this->pdf->MultiCell(30, $heigh,ucfirst(strtolower($row->t_name)),  1, 'L', 0, 0, '', '', true, 0, false, true, 0);
			...
			$this->pdf->MultiCell(30, $heigh,$row->amount,  1, 'R', 0, 0, '', '', true, 0, false, true, 0);
			$this->pdf->MultiCell(30, $heigh,$row->payment, 1, 'R', 0, 1, '', '', true, 0, false, true, 0);
		...
		$this->pdf->Cell(90, 1, $memo, '0', 0, 'L', 0);
			...
			$this->pdf->Cell(40, 1,"Rs.    ".number_format($num,2), 'TB', 0, 'L', 0);
			$this->pdf->Ln(2);
			$this->pdf->SetFont('helvetica','B',9);
			...
			if($issue_chq!=2){
				...
				$this->pdf->Cell(65, 2,"Bank Name", '1', 0, 'L', 0);
				...
				foreach ($issue_chq as $cc) {
					...
					$this->pdf->Cell(65, 2,$b_code[0], '1', 0, 'L', 0);
			...
			$this->pdf->Cell(60, 1,"", '0', 0, 'R', 0);
			$this->pdf->Cell(25, 1, "Operator", '0', 0, 'L', 0);
			$this->pdf->Cell(20, 1,$operator, '0', 0, 'R', 0);
			...
			$this->pdf->Cell(60, 1,"", '0', 0, 'R', 0);
			$this->pdf->Cell(40, 1, "Print Time", '0', 0, 'L', 0);
			$this->pdf->Cell(20, 1,$tt, '0', 0, 'R', 0);

<?php if($this->user_permissions->is_view('t_bankrec_additional_det')){ ?>
<script type='text/javascript' src='<?=base_url()?>js/t_bankrec_additional_det.js'></script>
<link rel="stylesheet" type="text/css" href="<?=base_url()?>css/t_forms.css" />
<h2 id="form_name">Undifined Deposit</h2>
<div class="dframe" id="mframe" style="margin-top:10px; width:984px; padding-right:25px;">
    <form method="post" action="<?=base_url()?>index.php/main/save/t_bankrec_additional_det" id="form_">
            ...
            <tr>
                <td colspan="2" style="text-align: center;">
                    <div style="text-align:left; padding-top: 7px;">
                        ...
                        <?php if($this->user_permissions->is_re_print('t_bankrec_additional_det')){ ?><input type="button" id="btnPrint" title="Print" /><?php } ?> 
                        <?php if($this->user_permissions->is_add('t_bankrec_additional_det')){ ?><input type="button" id="btnSave" title="Save" /><?php } ?>
                    </div>
                </td>
            </tr>
        </table>
         <?php 
    if($this->user_permissions->is_print('t_bankrec_additional_det')){ ?>
        <input type="hidden" name='is_prnt' id='is_prnt' value="1" title="1">
    <?php } ?> 
    </form>
    </form>
      <form id="print_pdf" action="<?php echo site_url(); ?>/reports/generate" method="post" target="_blank">      
        <input type="hidden" name='by' value='t_bankrec_additional_det' title="t_bankrec_additional_det" class="report">

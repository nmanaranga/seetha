...
$this->pdf->SetFont('helvetica', 'B', 16);
    ...
    $this->pdf->Cell(0, 5, 'Supplier wise Sales Report  ',0,false, 'L', 0, '', 0, false, 'M', 'M');
    ...
    if($supplier!=""){
      ...
      $this->pdf->SetFont('helvetica', '', 8);    
      $this->pdf->Cell(20, 1,": ".$supplier_des, '0', 0, 'L', 0);
    }else{
      $this->pdf->Ln();       
      $this->pdf->SetFont('helvetica', 'B', 9);
      $this->pdf->Cell(25, 1, "Supplier", '0', 0, 'L', 0);
    ...
    $this->pdf->setX(15);
    $this->pdf->Cell(30, 6,"Supplier Code", '1', 0, 'C', 0);
    $this->pdf->Cell(100, 6,"Supplier Name", '1', 0, 'C', 0);
    ...
    foreach ($r_data as $value) {
      $heigh=6*(max(1,$this->pdf->getNumLines($value->supplier,35),$this->pdf->getNumLines($value->name, 80)));
      ...
      $this->pdf->HaveMorePages($heigh);
      $this->pdf->MultiCell(30, $heigh,$value->supplier,'1', 'L', false, 0, '', '', true, 0, false, true, $heigh,'M' ,false);
      $this->pdf->MultiCell(100, $heigh,$value->name,'1', 'L', false, 0, '', '', true, 0, false, true, $heigh,'M' ,false);
    ...
    $this->pdf->Output("Supplier wise Sales Report_".date('Y-m-d').".pdf", 'I');

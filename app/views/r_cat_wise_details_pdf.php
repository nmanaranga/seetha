...
$this->pdf->SetFont('helvetica', 'B', 16);
    ...
    $this->pdf->Cell(0, 5, 'Category wise Details Report  ',0,false, 'L', 0, '', 0, false, 'M', 'M');
    ...
    foreach ($r_data as $value) {
        ...
        if($rep=='default' || $rep!=$value->rep){
            ...
            if($x!=0){
                ...
                $this->pdf->Cell(25, 6,'Sub Total ', '0', 0, 'L', 0);
            ...
            if($x!=0){
                ...
                $this->pdf->Cell(20, 6,number_format($rprice,2), 'TB', 0, 'R', 0);   
                $this->pdf->Cell(20, 6,number_format($rdiscount,2), 'TB', 0, 'R', 0);
                $this->pdf->Cell(20, 6,number_format($ramount,2), 'TB', 0, 'R', 0);  
            }
            $this->pdf->Ln();
            $rprice=0;
            $rdiscount=0;
            ...
            $this->pdf->MultiCell(0, $heigh,'','T', 'L', false, 0, '', '', true, 0, false, true, $heigh,'M' ,false);
            $this->pdf->Ln();
            $this->pdf->HaveMorePages($heigh);
            ...
            if($s_cat=='default_cat' || $s_cat!=$value->category){

                $this->pdf->HaveMorePages($heigh);
                $this->pdf->SetFont('helvetica','B',10);
                $this->pdf->MultiCell(20, $heigh,'','0', 'L', false, 0, '', '', true, 0, false, true, $heigh,'M' ,false);
                $this->pdf->MultiCell(0, $heigh,$value->cat_name,'0', 'L', false, 0, '', '', true, 0, false, true, $heigh,'M' ,false);
                $this->pdf->Ln();

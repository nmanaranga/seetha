<?php if($this->user_permissions->is_view('g_internel_transfer_receive')){ ?>
<link rel="stylesheet" type="text/css" href="<?=base_url()?>css/t_forms.css" />
<script type="text/javascript" src="<?=base_url()?>js/g_internel_transfer_receive.js"></script>
...
<h2 style="text-align: center;">Gift Voucher Internal Transfer Receive</h2>
<div class="dframe" id="mframe" style="padding-right:25px; width: 1040px;">
    <form method="post" action="<?=base_url()?>index.php/main/save/g_internel_transfer_receive" id="form_">
                ...
                <td  rowspan='3'  valign="top" align="right">
                    <table>
                        ...
                        <tr>
                            <td>Date</td>
                            <td>
                            <?php if($this->user_permissions->is_back_date('g_internel_transfer_receive')){ ?>
 ...
 -->
                            ...
                            <?php
                                ...
                                for($x=0; $x<25; $x++){
                                    echo "<tr>";
                                        ...
                                        echo "<td width='70' style='background-color: #f9f9ec;'><input type='button'  class='quns' id='btn_".$x."' style='margin:0;padding:5px;float: left; height:6px;width:6px;cursor:pointer'/>
                                              <input type='text' readonly='readonly' class='g_input_num qun chk ky qtycl".$x."' id='2_".$x."' name='2_".$x."' style='width:40px; float:right;'/></td>";
                ...
                <input type='hidden' id='transCode' value='64' title='64'/>
                <div style="text-align: right; padding-top: 7px; margin-right:2px;">
                    <input type="button" id="" title="Exit" />
                    <input type="button" id="btnReset" title="Reset" />
                    <?php if($this->user_permissions->is_delete('g_internel_transfer_receive')){ ?><input type="button" id="btnDelete" title="Cancel" /><?php } ?>
                    <?php if($this->user_permissions->is_re_print('g_internel_transfer_receive')){ ?><input type="button" id="btnPrint" title="Print" /><?php } ?>
			        <?php if($this->user_permissions->is_add('g_internel_transfer_receive')){ ?><input type="button" title='Save <F8>' value="Save" id="btnSave" /><?php } ?>
                    ...
                    <?php 
                    if($this->user_permissions->is_print('g_internel_transfer_receive')){ ?>
    ...
    <form id="print_pdf" action="<?php echo site_url(); ?>/reports/generate" method="post" target="_blank">
        ...
        <input type="hidden" name='by' value='g_internel_transfer_receive' title="g_internel_transfer_receive" class="report">
        <input type="hidden" name='page' value='A4' title="A4" >
        <input type="hidden" name='orientation' value='P' title="P" >
        <input type="hidden" name='type' value='g_internel_transfer_receive' title="g_internel_transfer_receive" >         

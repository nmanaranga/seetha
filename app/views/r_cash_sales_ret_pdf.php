...
$this->pdf->Cell(0, 5, 'CASH SALES RETURN SUMMARY',0,false, 'C', 0, '', 0, false, 'M', 'M');
$this->pdf->Ln();
$this->pdf->Ln();
...
else
{
	...
	if(!empty($cancel)){
		$c_gross=$c_dis=$c_net=0;
		$this->pdf->Ln();
		$this->pdf->SetFont('helvetica','B',9);
		$this->pdf->SetX(15);
		$this->pdf->Cell(10, 6,"CANCELLED RETURNS", '0', 0, 'L', 0);
		$this->pdf->Ln();
		$this->pdf->Ln();
		...
		foreach ($cancel as $value) {	
			...
			$this->pdf->MultiCell(25, $heigh,$value->store, 1, 'L', 0, 1, '', '', true, 0, false, true, 0);
			$i++;
			$c_gross+=$value->gross_amount;
			$c_dis+=$value->discount;
		...
		$this->pdf->Cell(20, 6,"", '0', 0, 'L', 0);
		$this->pdf->Cell(60, 6,"Total ", '0', 0, 'R', 0);
		$this->pdf->Cell(22, 6,number_format($c_gross,2), 'TB', 0, 'R', 0);
		$this->pdf->Cell(1, 6,"", '0', 0, 'R', 0);
		$this->pdf->Cell(21, 6,number_format($c_dis,2), 'TB', 0, 'R', 0);
		$this->pdf->Cell(1, 6,"", '0', 0, 'R', 0);
		$this->pdf->Cell(22, 6,number_format($c_net,2), 'TB', 0, 'R', 0);
		$this->pdf->Cell(22, 6,"", '0', 0, 'R', 0);
	}

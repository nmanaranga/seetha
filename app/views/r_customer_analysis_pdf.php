...
$this->pdf->SetFont('helvetica','B',8);
   /* $this->pdf->Cell(22, 6," Customer ID", '1', 0, 'C', 0);
    ...
    $this->pdf->Cell(20, 6,"unsettle Amo ", '1', 0, 'C', 0);
    ...
    foreach($cus_det as $row){
        $aa = $this->pdf->getNumLines($row->NAME, 55);
        $bb = $this->pdf->getNumLines($row->nic, 20);
        if($aa>$bb){
            $heigh=5*$aa;
        }else{
         $heigh=5*$bb; 
     ...
     $this->pdf->SetX(10);
     $this->pdf->MultiCell(20, $heigh,$row->acc_code,  1, 'L', 0, 0, '', '', true, 0, false, true, 0);
     $this->pdf->MultiCell(55, $heigh,ucfirst(strtolower($row->NAME)),  1, 'L', 0, 0, '', '', true, 0, false, true, 0);
     ...
     $this->pdf->MultiCell(20, $heigh,$row->Over90,  1, 'R', 0, 0, '', '', true, 0, false, true, 0);
     $this->pdf->MultiCell(20, $heigh,$row->unSettle,  1, 'R', 0, 1, '', '', true, 0, false, true, 0);
        //$this->pdf->Ln();
 ...
 $this->pdf->Cell(1, 6,"", '0', 0, 'L', 0);
 $this->pdf->Cell(19, 6,number_format($total5,2), 'TB', 0, 'R', 0);

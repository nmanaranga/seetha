...
///////
		...
		if(!empty($_POST['cluster'])){
			$cluster = "AND t_serial.cl='".$_POST['cluster']."'";
		}else{
			$cluster="";
		}
		if(!empty($_POST['branch'])){
			$branch = "AND t_serial.bc='".$_POST['branch']."'";
		}else{
			$branch="";
		}
		if(!empty($_POST['store'])){
			$store = "AND t_serial.store_code='".$_POST['store']."'";
		}else{
			$store="";
		}
		if(!empty($_POST['item'])){
			$item = "AND t_serial.item='".$_POST['item']."'";
		}else{
			$item="";
		}
		...
		INNER JOIN (SELECT cl,bc,item,qty AS item_tot,batch_no FROM qry_current_stock  GROUP BY item) c ON t_serial.item = c.item AND t_serial.batch = c.batch_no AND t_serial.cl=c.cl AND t_serial.bc=c.bc
		WHERE t_serial.`available` = '1' 
		AND m_item.`serial_no` ='1'
		$cluster
		$branch
		$store
		$item
		GROUP BY t_serial.`item`";

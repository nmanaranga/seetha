...
class m_allocate_bank_branch extends CI_Model {
  ...
  public function base_details(){
    ...
    $a['max_no']= $this->utility->get_max_no("m_allocate_bank_branch","nno");
  ...
  public function save(){
    ...
    try {
      ...
      $this->max_no = $this->utility->get_max_no("m_allocate_bank_branch", "nno");
      ...
      if($validation_status==1){
        ...
        for($x = 0; $x<$_POST['hid_tot']; $x++){
          if(isset($_POST['bank_acc'],$_POST['u_cluster'],$_POST['active_'.$x])){
            if($_POST['active_'.$x] == 1){
              $m_allocate_bank_branch[]=array(
               "nno"=>$this->max_no,
               "ddate"=>$_POST['date'],
               "bank_id"=>$_POST['bank_acc'],
               "cl"=>$_POST['cl_'.$x],
               "bc"=>$_POST['bc_'.$x],
               "is_active"=>$_POST['active_'.$x]  
             );    
            }
          }
        }
      ...
      //echo '<pre>'.print_r($m_allocate_bank_branch,true).'</pre>';
        if($_POST['hid_user'] == "1"){
          if($this->user_permissions->is_add('m_allocate_bank_branch')){
            if (isset($m_allocate_bank_branch)) {
              if(count($m_allocate_bank_branch)){$this->db->insert_batch("m_allocate_bank_branch",$m_allocate_bank_branch);}
            }
          ...
          if($this->user_permissions->is_edit('m_allocate_bank_branch')){
            $this->set_delete();
            if (isset($m_allocate_bank_branch)) {
              if(count($m_allocate_bank_branch)){$this->db->insert_batch("m_allocate_bank_branch",$m_allocate_bank_branch);}
            }
  ...
  private function set_delete(){
    $this->db->where("bank_id", $_POST['bank_acc']);
    $this->db->delete("m_allocate_bank_branch");
...
public function check_user(){
  ...
  $sql="SELECT * FROM m_allocate_bank_branch WHERE bank_id='$bank'";
...
public function load_exist_bc_detail(){
  ...
  if($cluster==""){
    ...
    FROM m_allocate_bank_branch b
    RIGHT JOIN m_branch ON m_branch.`bc` = b.`bc` 
    LEFT JOIN m_cluster ON m_cluster.`code` = m_branch.`cl`
    LEFT JOIN (SELECT cl,bc,is_active FROM m_allocate_bank_branch WHERE bank_id='$bank') n ON n.cl= b.cl 
    AND n.bc= b.bc
    GROUP BY cl,bc";
  }else{
    $sql="SELECT  m_branch.cl, 

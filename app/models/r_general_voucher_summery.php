...
class r_general_voucher_summery extends CI_Model{
  ...
  public function PDF_report(){
   ...
   $sql1="SELECT
     ...
     if(!empty($_POST['from']))
        {
           $sql1.=" AND t_voucher_gl_sum.ddate between '".$_POST['from']."' AND '".$_POST['to']."'";
        }
       // if(!empty($_POST['acc_code']))
       //  {
       //     $sql.=" AND MA.`code` = '$acc'";
       //  }
        ...
        if(!empty($cluster))
        {
           $sql1.=" AND t_voucher_gl_sum.cl = '$cluster'";
        }
       if(!empty($branch))
        {
            $sql1.=" AND t_voucher_gl_sum.bc = '$branch'";
        }
         
        if(!empty($_POST['t_range_from']) && empty($_POST['t_range_to']))
        {
           $sql1.=" AND t_voucher_gl_sum.`nno` >= '$no_range_frm'";
        }
        ...
        if(!empty($_POST['t_range_to']) && !empty($_POST['t_range_to']))
        {
           $sql1.=" AND t_voucher_gl_sum.`nno` BETWEEN  '$no_range_frm' AND '$no_range_to'";
        }

        $sql1.="ORDER BY t_voucher_gl_sum.cl,t_voucher_gl_sum.bc,t_voucher_gl_sum.nno ASC";

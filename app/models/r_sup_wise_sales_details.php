...
class r_sup_wise_sales_details extends CI_Model{
  ...
  public function PDF_report($RepTyp=""){
    ...
    s.`category`,
    s.description,
    d.`description` AS dep_des,
    c.`description` AS main_cat,
    sc.`description` AS cat_name,
    SUM(s.amount) AS amount,
    SUM(s.qty) AS qty,
    s.supplier,
    su.`name`
    FROM
    (SELECT i.`department`,i.`main_category`,i.`category`,i.`description`,cs.cl,cs.bc,cs.`ddate`,'CASH' AS  type,cs.`nno`,cd.`code`,cd.`price`,cd.`discount`,cd.qty,cd.`amount`,cs.`rep`,i.`supplier` 
    FROM `t_cash_sales_det` cd 
    JOIN `t_cash_sales_sum` cs ON cs.`cl`=cd.`cl` AND cs.`bc`=cd.`bc` AND cs.`nno`=cd.`nno`
    JOIN `m_item` i ON i.`code` = cd.code 
    ...
    SELECT i.`department`,i.`main_category`,i.`category`,i.`description`,cs.cl,cs.bc,cs.`ddate`,'CREDIT' AS  type,cs.`nno`,cd.`code`,cd.`price`,cd.`discount`,cd.qty,cd.`amount`,cs.`rep`,i.`supplier` 
    ...
    JOIN `m_supplier` su ON su.`code`=s.supplier
    JOIN `r_department` d ON d.`code` = s.`department` 
    JOIN `r_category` c ON c.`code` = s.`main_category` 
    JOIN r_sub_category sc ON sc.code = s.`category` 
    AND sc.`main_category` = s.`main_category` 
    ...
    if($_POST['main_category']!=""){
      $sql.=" AND s.`main_category`='".$_POST['main_category']."'";
    }

    if($_POST['sub_category']!=""){
      $sql.=" AND s.`category`='".$_POST['sub_category']."'";
    }
    ...
    $sql.="GROUP BY s.supplier,s.`department`,s.`main_category`,s.`category`  ";
    $sql.=" ORDER BY s.supplier,s.`department` ";

...
///////
		...
		if(isset($_POST['minus'])){
			$qty_chk='IFNULL(i.qty,0)<0';
		}else{
			$qty_chk='IFNULL(i.qty,0)>0';
		}

		$sql="SELECT m.`code` AS item	, IFNULL(i.batch_no,0) AS batch_no
				, IFNULL(c.item_tot,0) AS item_tot, IFNULL(i.store_code,'') AS store_code
				, IFNULL(i.qty,0) AS qty
				, IFNULL(sr.sr_count,0) AS sr_count, IFNULL(i.cost,0) AS cost, m.`description`, m.model, IFNULL(i.p_date,'') AS p_date, m.purchase_price AS p_price
				, m.`max_price` AS m_price, m.`min_price` AS l_price, e.purchase_price AS b_price, e.min_price AS b_min, e.max_price AS b_max 
			FROM `m_item` `m` 
			LEFT JOIN (SELECT item, batch_no, cost, store_code
						, SUM(qty_in) - SUM(qty_out) AS qty, MIN(ddate) AS p_date 
						FROM t_item_movement 	WHERE ddate <='$to'  $clu $bru $str $itm
						GROUP BY item, batch_no, store_code	HAVING qty>0 ) i ON ((i.`item` = m.`code`)) 
	 		LEFT JOIN t_item_batch e ON i.item=e.`item` AND i.batch_no =e.batch_no		
						LEFT JOIN (SELECT item,SUM(qty_in - qty_out) AS item_tot,`store_code`,cl,bc 
						FROM t_item_movement 	WHERE (t_item_movement.cl = '$clu2' AND t_item_movement.bc = '$bru2'  ) GROUP BY item, cl, bc) c ON m.`code` = c.item 
			LEFT JOIN (SELECT item,batch, COUNT(serial_no) AS sr_count FROM t_serial 
						WHERE available='1' $clu $bru $str  GROUP BY item,batch	) AS sr ON sr.item=m.`code` 
						AND sr.batch=i.batch_no 
			WHERE $qty_chk $dep $main_cat $sub_cat $unit $brand $sup
			order by m.`code` , IFNULL(i.batch_no,0)";
/*
	$sql="SELECT m.`code` AS item, 
				...
				IFNULL(i.qty,0) AS qty, 
				IFNULL(sr.sr_count,0) AS sr_count,
				...
				LEFT JOIN  (SELECT item, batch_no, cost, store_code, 
					...
					JOIN (SELECT item,batch_no,purchase_price,min_price,max_price FROM t_item_batch 
						GROUP BY item,batch_no) e ON e.item = m.code AND e.batch_no=i.batch_no
					LEFT JOIN   (SELECT item,SUM(qty_in - qty_out) AS item_tot,`store_code`,cl,bc 
						FROM t_item_movement WHERE (
					t_item_movement.bc = '$bru2'  AND t_item_movement.cl = '$clu2' $str  ) 
					GROUP BY item, cl, bc) c ON m.`code` = c.item
				LEFT JOIN (SELECT item,batch, COUNT(serial_no) AS sr_count
					FROM t_serial 
					WHERE available='1' $clu $bru $str GROUP BY item,batch) AS sr ON sr.item=m.`code` AND sr.batch=i.batch_no  

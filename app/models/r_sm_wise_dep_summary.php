...
class r_sm_wise_dep_summary extends CI_Model{
  ...
  public function PDF_report($RepTyp=""){
    ...
    SUM(s.qty) AS qty,s.rep,
    e.`name`
    FROM
    (SELECT cs.cl,cs.bc,cs.`ddate`,'CASH' AS  type,cs.`nno`,cd.`code`,cd.`price`,cd.`discount`,cd.qty,cd.`amount`,cs.`rep` 
    ...
    SELECT cs.cl,cs.bc,cs.`ddate`,'CREDIT' AS  type,cs.`nno`,cd.`code`,cd.`price`,cd.`discount`,cd.qty,cd.`amount`,cs.`rep` 
    ...
    AND sc.`main_category` = i.`main_category` 
    ...
    $sql.=" GROUP BY s.rep,i.`department` ";
    $sql.=" ORDER BY s.rep,i.`department` ";

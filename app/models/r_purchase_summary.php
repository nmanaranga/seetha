    ...
    public function base_details(){
     ...
     public function PDF_report($RepTyp=""){
          ...
          $query1=$this->db->query('SELECT sum(d.`qty` * d.`price`) as gsum ,sum(d.`amount`+ g.`additional`) as nsum, sum(g.`additional`) as addi
            ...
            WHERE g.`cl`="'.$_POST['cluster'].'" AND g.`bc`= "'.$_POST['branch'].'"
        	and g.ddate between "'.$_POST['from'].'" and "'.$_POST['to'].'"');
          $r_detail['sum']=$query1->result();
          
          $query = $this->db->query('SELECT g.`nno` ,g.`ddate` ,g.`supp_id`  ,s.`name` , d.`discount` , inv_date,   
            g.gross_amount AS `gross_amount`,
            g.net_amount AS amount,g.`inv_no` ,m.`description` , g.`memo`
				FROM `t_grn_sum` g
				LEFT JOIN `m_supplier` s ON g.`supp_id` = s.`code` 
				LEFT JOIN `t_grn_det` d ON g.`nno` = d.`nno` AND g.cl = d.cl AND g.bc = d.bc
				LEFT JOIN `m_stores` m ON g.`store` = m.`code`
				WHERE g.`cl`="'.$_POST['cluster'].'" AND g.`bc`= "'.$_POST['branch'].'" 
        and g.ddate between "'.$_POST['from'].'" and "'.$_POST['to'].'" GROUP BY g.cl,g.bc,g.`nno` 
         order by g.nno' );

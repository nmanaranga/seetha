...
public function PDF_report($RepTyp=""){
  ...
  'CASH' AS s_type,
  tx.rate,
  IFNULL(rsd.amount,0) AS ret_price,
  IFNULL(rsd.qty,0) AS ret_qty
  ...
  LEFT JOIN (SELECT rd.cl,rd.bc,rd.`code`,rs.`inv_no`,rd.`qty`,rd.`amount`,rs.ddate FROM t_sales_return_sum rs
  JOIN t_sales_return_det rd ON rd.cl=rs.`cl` AND rd.bc=rs.`bc` AND rd.`nno`=rs.`nno` WHERE rs.`sales_type`='4' AND rs.ddate BETWEEN '$f_date' AND '$t_date' )rsd ON rsd.cl=s.cl AND rsd.bc=s.bc AND rsd.code=d.code AND rsd.inv_no=s.nno
  ...
  JOIN m_tax_setup tx ON tx.code=m.tax_rate
  LEFT JOIN (SELECT rd.cl,rd.bc,rd.`code`,rs.`inv_no`,rd.`qty`,rd.`amount`,rs.ddate FROM t_sales_return_sum rs
  JOIN t_sales_return_det rd ON rd.cl=rs.`cl` AND rd.bc=rs.`bc` AND rd.`nno`=rs.`nno` WHERE rs.`sales_type`='5' AND rs.ddate BETWEEN '$f_date' AND '$t_date')rsd ON rsd.cl=s.cl AND rsd.bc=s.bc AND rsd.code=d.code AND rsd.inv_no=s.nno
  WHERE s.ddate BETWEEN '$f_date' AND '$t_date' AND s.is_cancel='0' AND d.is_free='0' $cl $bc $item2
  GROUP BY d.nno,d.`code`,d.batch_no
  ORDER BY s_type";
  ...
  UNION ALL SELECT  d.cl,d.bc,d.`qty`,d.`amount`,s.ddate  FROM t_sales_return_sum s JOIN t_sales_return_det d ON d.cl=s.`cl` AND d.bc=s.`bc` AND d.`nno`=s.`nno` WHERE s.ddate BETWEEN '$f_date' AND '$t_date'  $cl $bc GROUP BY ddate
  ";

...
public function set_delete(){
  ...
  $this->trans_settlement->delete_settlement_sub("t_credit_note_trans_branch","135",$this->max_no);   
  $this->trans_settlement->delete_settlement_sub("t_debit_note_trans_branch","135",$this->max_no); 
...
public function delete(){
  ...
  try { 
    if($this->user_permissions->is_delete('t_branch_settlement')){
      ...
      $this->trans_settlement->delete_settlement_sub("t_credit_note_trans_branch",135,$trans_no); 
      $this->trans_settlement->delete_settlement_sub("t_debit_note_trans_branch",135,$trans_no);   
...
public function load_item_cr(){
  ...
  $sql="SELECT  tc.`description`, d.* FROM (
  SELECT s.nno,s.sub_no,t.bc,t.sub_cl, t.sub_bc, t.trans_code AS TYPE,t.trans_no, MIN(t.ddate) ddate, SUM(t.`dr`) AS amount,s.balance AS balance 
...
public function item_list_all(){
  ...
  if($this->sd['branch']=='MS'){
   ...
   WHERE (t.acc_code='$acc_code' AND t.`trans_code` IN ('17','134')) AND t.sub_cl= s.cl AND t.sub_bc = s.bc AND s.is_cancel='0' GROUP BY t.sub_cl,t.sub_bc, t.trans_no, t.trans_code

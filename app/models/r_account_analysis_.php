...
class r_account_analysis extends CI_Model {
    ...
    public function PDF_report($RepTyp=""){
        ...
        CONCAT(t_account_trans.cl,t_account_trans.`bc`,LPAD(t_account_trans.trans_no,6,0)) AS tno , a.acc_code,a.op_ac
        FROM t_account_trans LEFT JOIN t_trans_code ON t_trans_code.`code`= t_account_trans.`trans_code` 

        LEFT JOIN(SELECT t_account_trans.*,ac.`description` AS op_ac FROM t_account_trans 
        JOIN m_account ac ON ac.`code`=t_account_trans.`acc_code`
        WHERE ddate BETWEEN '$_POST[from]' AND '$_POST[to]'  AND acc_code NOT IN ('$_POST[acc_code]'))a 
        ON a.cl= t_account_trans.`cl` AND a.bc=t_account_trans.`bc` AND a.trans_code=t_account_trans.`trans_code` AND a.trans_no=t_account_trans.`trans_no`


        WHERE t_account_trans.ddate!='' 
        ...
        $sqll.=" ORDER BY t_account_trans.ddate,t_account_trans.auto_no";

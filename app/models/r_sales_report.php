...
public function PDF_report()
{
    ...
    IFNULL(po.poAmount, 0) AS PosSales,
    IFNULL(po.discount_total, 0) AS PosDis,
    (IFNULL(po.poAmount, 0) - IFNULL(po.discount_total, 0)) AS TotPos 
    FROM `r_department` d
    LEFT JOIN (
        ...
        LEFT JOIN (
            ...
            LEFT JOIN (
                SELECT i.`department`,SUM(d.`amount`) AS poAmount,SUM(s.`total_discount`) AS discount_total 
                FROM `t_pos_sales_sum` s 
                INNER JOIN `t_pos_sales_det` d ON s.`cl` = d.`cl` AND s.`bc` = d.`bc` AND s.`nno` = d.`nno` 
                INNER JOIN `m_item` i ON d.`item` = i.`code` 
                WHERE s.`ddate` BETWEEN '".$_POST['from']."' AND '".$_POST['to']."'  $cluster $branch
                GROUP BY  i.`department` ) AS po ON d.`code`=po.department  ";

...
class r_credit_sales_summary_2 extends CI_Model{
  ...
  public function PDF_report($RepTyp=""){
    ...
    if($_POST['group_id']!=""){
      $sql=" AND t_credit_sales_sum.group_no ='".$_POST['group_id']."' ";
    ...
    if($_POST['sales_category']!="0"){
      ...
      $query1=$this->db->query('SELECT SUM(`t_credit_sales_sum`.gross_amount) AS gsum,t_credit_sales_sum.additional_add,t_credit_sales_sum.additional_deduct,
        ...
        WHERE t_credit_sales_sum.is_cancel=0 AND t_credit_sales_sum.category ="'.$_POST['sales_category'].'" AND `t_credit_sales_sum`.`cl`="'.$this->sd['cl'].'" AND `t_credit_sales_sum`.`bc`= "'.$this->sd['branch'].'"
        and `t_credit_sales_sum`.`ddate` between "'.$_POST['from'].'" and "'.$_POST['to'].'" '.$sql.'');
    ...
    $cancel_query = $this->db->query('SELECT  s.`nno`,
      s.`sub_no`,
      s.`cus_id`,
      c.`name`,
      s.`ddate`,
    ...
    $r_detail['cancel']=$cancel_query->result();   

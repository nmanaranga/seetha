...
class loder extends CI_Model {
  ...
  public function base_details(){
      ...
      if(isset($_GET['action'])){
        $uurl_code=$_GET['action'];
        $url_output = str_replace('_', ' ', $uurl_code);
        $result_url = substr($url_output, 2);
        $f_result = ucwords($result_url);
        $ff_code=str_replace('Sum',' ',$f_result);
    }else{
        $ff_code="Welcome";
    }
        ...
        // $this->sd = $this->session->all_userdata();
    $menu='';
    // if (!isset($this->sd['menu'])) {$this->sd['menu']=true;}
...
public function default_settings(){
  ...
  foreach($query->result() as $row){
      ...
      $a['def_salesman_code']=$row->def_salesman_code;
      $a['use_collection_officer']=$row->use_collection_officer;
      $a['def_collection_officer_code']=$row->def_collection_officer_code;
  }

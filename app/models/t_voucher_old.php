...
class t_voucher extends CI_Model {
    ...
    public function validation(){
      ...
      $this->max_no=$this->utility->get_max_no("t_voucher_sum","nno");
        ...
        $check_valid_dr_no=$this->validation->check_valid_trans_no2('supplier_id','t_code_','no2_','cl_','bc_');
        ...
        $check_valid_dr_no2=$this->validation->check_valid_trans_no2('supplier_id','t_code3_','no3_','cl3_','bc3_');
              ...
              if(count($t_voucher_temp)){$this->db->insert_batch("t_voucher_det",$t_voucher_temp);}
                ...
                if($this->user_permissions->is_edit('t_voucher')){
                  ...
                  for($x = 0; $x<25; $x++){
                    if(isset($_POST['bc0_'.$x],$_POST['4_'.$x],$_POST['5_'.$x],$_POST['6_'.$x])){
                      if($_POST['bc0_'.$x] != "" && $_POST['4_'.$x] != "" && $_POST['5_'.$x] != "" && $_POST['6_'.$x]!= ""){
                        $this->trans_settlement->delete_settlement_sub("t_sup_settlement",$_POST['trans_code'.$x],$this->max_no);
                        $this->trans_settlement->delete_settlement_sub("t_credit_note_trans",$_POST['trans_code'.$x],$this->max_no);
  ...
  public function account_update($condition){
    ...
    if(isset($_POST['cheque_issue']) && !empty($_POST['cheque_issue']) && $_POST['cheque_issue']>0){
      ...
      $this->account->set_value2($des, $_POST['cheque_issue'], "cr", $acc_code,$condition);    
    }
    ...
    public function load_supplier_details(){
      ...
      //   SELECT t.cl, t.bc, t.trans_code AS TYPE,t.trans_no, MIN(t.ddate) ddate, s.memo,s.net_amount AS amount,SUM(t.dr)-SUM(t.cr) balance 
      //   FROM `t_sup_settlement` t
      //   INNER JOIN `t_grn_sum` s ON t.trans_no=s.nno AND t.cl= s.cl AND t.bc = s.bc
      //   WHERE (t.trans_code='3') AND (acc_code='$acc_code') AND(t.cl='$cl') AND (t.bc='$branch') GROUP BY trans_no, t.trans_code,t.cl,t.bc
      //   HAVING balance > 0
      //   UNION ALL 
      //   SELECT t.cl, t.bc, t.trans_code AS trans_code_no,t.trans_no, MIN(t.ddate) ddate,s.memo, s.amount ,SUM(t.dr)-SUM(t.cr) balance 
      //   FROM `t_credit_note_trans` t
      //   INNER JOIN `t_credit_note` s ON t.trans_no=s.nno AND t.cl= s.cl AND t.bc = s.bc
      //   WHERE (t.trans_code='17') AND (t.acc_code='$acc_code') AND(t.cl='$cl') AND (t.bc='$branch') GROUP BY trans_no, t.trans_code,t.cl,t.bc
      //   ) d INNER JOIN t_trans_code tc ON tc.`code` = d.type WHERE d.cl ='$cl' AND d.bc = '$branch'
      //   HAVING balance > 0";


        // $sql="SELECT tc.`description`, d.* FROM( 
        //     SELECT s.cl, s.bc, s.type, s.nno AS trans_no, s.memo, s.ddate, s.net_amount AS amount,t.balance 
        //     FROM t_grn_sum s
        //     INNER JOIN
        //     (SELECT trans_code,trans_no, acc_code, SUM(dr)-SUM(cr) balance FROM t_sup_settlement  
        //     WHERE acc_code='$acc_code'  AND cl='$cl' AND bc='$bc' AND trans_code='3'
        //     GROUP BY trans_no HAVING SUM(dr)-SUM(cr)>0)t ON t.trans_no=s.nno AND t.acc_code=s.supp_id
        //     WHERE s.supp_id='$acc_code' AND cl='$cl' AND bc='$bc' AND t.trans_code='3'
        //     UNION ALL 
        //     SELECT s.cl, s.bc, s.type,s.nno AS trans_no, s.memo, s.ddate, s.amount,t.balance 
        //     FROM t_credit_note s
        //     INNER JOIN 
        //     (SELECT trans_no,acc_code,SUM(dr)-SUM(cr) AS balance FROM `t_credit_note_trans` WHERE acc_code='$acc_code'  AND cl='$cl' AND bc='$bc'
        //     GROUP BY trans_no HAVING balance > 0) t ON t.trans_no=s.nno AND t.acc_code=s.code
        //     WHERE s.code='$acc_code' AND cl='$cl' AND bc='$bc'
        //     ) d 
   ...
   public function load_supplier_details_all(){
      ...
      //   FROM `t_sup_settlement` t
      //   INNER JOIN `t_grn_sum` s ON t.trans_no=s.nno AND t.cl= s.cl AND t.bc = s.bc
      //   WHERE (t.trans_code='3') AND (acc_code='$acc_code') GROUP BY trans_no, t.trans_code,t.cl,t.bc
      //   UNION ALL 
      //   SELECT t.cl, t.bc, t.trans_code AS trans_code_no,t.trans_no, MIN(t.ddate) ddate, s.memo,s.amount ,SUM(t.dr)-SUM(t.cr) balance 
      //   FROM `t_credit_note_trans` t
      //   INNER JOIN `t_credit_note` s ON t.trans_no=s.nno AND t.cl= s.cl AND t.bc = s.bc
      //   WHERE (t.trans_code='17') AND (t.acc_code='$acc_code') GROUP BY trans_no, t.trans_code,t.cl,t.bc
      ...
      //     GROUP BY trans_no HAVING SUM(dr)-SUM(cr)>0)t ON t.trans_no=s.nno AND t.acc_code=s.supp_id
      //     WHERE s.supp_id='$acc_code' 
      //     UNION ALL 
      //     SELECT s.cl, s.bc, s.type,s.nno AS trans_no, s.memo, s.ddate, s.amount,t.balance 
      //     FROM t_credit_note s
      //     INNER JOIN 
      //     (SELECT trans_no,acc_code,SUM(dr)-SUM(cr) AS balance FROM `t_credit_note_trans` WHERE acc_code='$acc_code'  
      //     GROUP BY trans_no HAVING balance > 0) t ON t.trans_no=s.nno AND t.acc_code=s.code
      //     WHERE s.code='$acc_code' 
      //     ) d 
      //     INNER JOIN t_trans_code tc ON tc.`code` = d.type "; 

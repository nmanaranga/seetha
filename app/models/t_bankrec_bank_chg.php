...
class t_bankrec_bank_chg extends CI_Model {
    ...
    try { 
      ...
      if ($validation_status == 1){
        ...
        if($_POST['hid'] == "0" || $_POST['hid'] == ""){
          ...
          if($this->user_permissions->is_edit('t_bankrec_bank_chg')){ 
            ...
            $this->account_update(1);
            $this->utility->save_logger("EDIT",94,$_POST['id'],$this->mod); 
            echo $this->db->trans_commit();
        }else{
 ...
 public function account_update($condition)
  {
    ...
    for ($i = 0; $i < 25; $i++) {
      if ($_POST['2_' . $i] != "" && $_POST['4_' . $i] != "0") {
        $dess = "BANK CHARGERS - " . $_POST['2_' . $i];
        $this->account->set_value2($dess, $_POST['4_'.$i], "cr", $bank_acc, $condition);
        $this->account->set_value2($dess, $_POST['4_'.$i], "dr", $_POST['6_'.$i], $condition);
      }
    }
    ...
    if ($condition == 0) {
      $query = $this->db->query("
           ...
           WHERE  t.`cl`='" . $this->sd['cl'] . "'  AND t.`bc`='" . $this->sd['branch'] . "'  AND t.`trans_code`='94'  AND t.`trans_no` ='" .$_POST['id']. "' AND 
           a.`is_control_acc`='0'");
...
public function load(){
  $sql="SELECT d.*, a.description as acc_des

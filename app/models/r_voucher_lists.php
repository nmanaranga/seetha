...
class r_voucher_lists extends CI_Model{
    ...
    public function PDF_report(){
      ...
      $sql="SELECT 
              ...
              GROUP_CONCAT(cd.cheque_no) AS chq_no 
            FROM `t_voucher_sum`
            INNER JOIN `m_account` AS MA ON MA.`code`=t_voucher_sum.`acc_code`
            INNER JOIN `m_branch` MBC ON t_voucher_sum.bc=MBC.bc
            INNER JOIN `m_cluster` MCL ON (MBC.`cl` = MCL.`code`)
            LEFT JOIN opt_issue_cheque_det cd ON cd.trans_code='19' AND cd.trans_no=t_voucher_sum.nno
                        AND cd.cl=t_voucher_sum.cl AND cd.bc=t_voucher_sum.bc
            WHERE `is_cancel`='0'  ";
       ...
       if(!empty($branch))
        ...
        $sql.=" GROUP BY t_voucher_sum.cl,t_voucher_sum.bc,t_voucher_sum.nno ORDER BY cl,bc,nno ASC";

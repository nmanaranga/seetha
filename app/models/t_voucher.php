      ...
      public function save(){
        ...
        try {
          ...
          if($validation_status==1){
            ...
            $obj = json_decode($_POST['moreinfo']);
            $tbl5= sizeof($obj->vou_details);
            for ($i = 0; $i < $tbl5; $i++) {
              ...
              $t_voucher_temp[] = array(
                ...
                "bc" => $this->sd['branch'],
                "nno" => $this->max_no,
                "sub_cl" => $obj->vou_details[$i]->sub_cl,
                "sub_bc" => $obj->vou_details[$i]->sub_bc,
                "trans_code" => $obj->vou_details[$i]->trans_code,
                "trans_no" => $obj->vou_details[$i]->trans_no,
                "date" => $obj->vou_details[$i]->date,
                "amount" => $obj->vou_details[$i]->amount,
                "description" => $obj->vou_details[$i]->description,
                "balance" => $obj->vou_details[$i]->balance,
                "payment" => $obj->vou_details[$i]->payment,
                "order_num" => $obj->vou_details[$i]->order_num,
              );
           ...
           if($_POST['hid'] == "0" || $_POST['hid'] == ""){
            if($this->user_permissions->is_add('t_voucher')){
              ...
              if($account_update==1){
                ...
                for ($i = 0; $i < $tbl5; $i++) {
                  if(isset($obj->vou_details[$i]->sub_bc,$obj->vou_details[$i]->amount,$obj->vou_details[$i]->balance,$obj->vou_details[$i]->payment)){
                    if($obj->vou_details[$i]->sub_bc != "" && $obj->vou_details[$i]->amount != "" && $obj->vou_details[$i]->balance != "" && $obj->vou_details[$i]->payment!= ""){
                     if($obj->vou_details[$i]->trans_code=="3"){
                      $this->trans_settlement->save_settlement_multi("t_sup_settlement",$_POST['supplier_id'],$_POST['date'],$obj->vou_details[$i]->trans_code,$obj->vou_details[$i]->trans_no,19,$this->max_no,"0",$obj->vou_details[$i]->payment,$obj->vou_details[$i]->sub_cl,$obj->vou_details[$i]->sub_bc);  
                    }else if($obj->vou_details[$i]->trans_code=="17"){
                      $this->trans_settlement->save_settlement_multi("t_credit_note_trans",$_POST['supplier_id'],$_POST['date'],$obj->vou_details[$i]->trans_code,$obj->vou_details[$i]->trans_no,19,$this->max_no,"0",$obj->vou_details[$i]->payment,$obj->vou_details[$i]->sub_cl,$obj->vou_details[$i]->sub_bc);  
                    }else if($obj->vou_details[$i]->trans_code=="1"){
                      $this->trans_settlement->save_settlement_multi("t_opening_bal_trans",$_POST['supplier_id'],$_POST['date'],$obj->vou_details[$i]->trans_code,$obj->vou_details[$i]->trans_no,19,$this->max_no,$obj->vou_details[$i]->payment,"0",$obj->vou_details[$i]->sub_cl,$obj->vou_details[$i]->sub_bc);  
                    }            
                  }
                }
              }
          ...
          foreach($query->result() as $row){
            if($row->dr_amount>0){
              ...
              if($row->acc_code=='103000005005'){
                ...
                WHERE cl='".$this->sd['cl']."' AND bc='".$this->sd['branch']."' AND trans_code='19' AND trans_no='".$_POST['trans_no']."'";
                $query=$this->db->query($sql);
                $bank=$query->row()->bank;
...
public function load_supplier_details(){
  ...
  $bc=$this->sd['branch'];
     ...
     /* $sql="SELECT tc.`description`, d.* FROM( 
            ...
            JOIN t_opening_bal_sum s ON s.cl=tr.cl AND s.bc=tr.bc AND s.no=tr.trans_no
  ...
  public function PDF_report(){
    ...
    WHERE nno='".$_POST['qno']."'
    AND d.cl='".$this->sd['cl']."'
    AND d.bc='".$this->sd['branch']."' AND d.payment>0";
    ...
    $this->db->select(array('t_voucher_sum.oc', 's_users.discription','action_date'));
    $this->db->from('t_voucher_sum');

...
class t_bankrec_additional_det extends CI_Model {
    ...
    try { 
      ...
      if ($validation_status == 1){
            ...
            if(isset($_POST['4_'.$x])){  
              if($_POST['4_'.$x]!=""){  
                $t_bank_rec_add_det[]= array(
                    ...
                    "account"     =>$acc_code = $this->utility->get_default_acc('UNDIFINED_DEPOSIT'),
                    "oc"          =>$this->sd['oc'],
                );
              }        
            }
        ...
        if($_POST['hid'] == "0" || $_POST['hid'] == ""){
          ...
          if($this->user_permissions->is_edit('t_bankrec_additional_det')){ 
            ...
            $this->account_update(1);
            $this->utility->save_logger("EDIT",85,$_POST['id'],$this->mod); 
            echo $this->db->trans_commit();
        }else{
 ...
 public function account_update($condition)
  {
    ...
    $def_code = $this->utility->get_default_acc('UNDIFINED_DEPOSIT');

    for ($i = 0; $i < 25; $i++) {
      if ($_POST['2_' . $i] != "" && $_POST['4_' . $i] != "0") {
        $dess = "UNDIFINED DEPOSIT - " . $_POST['2_' . $i];
        $this->account->set_value2($dess, $_POST['4_'.$i], "dr", $bank_acc,$condition);
        $this->account->set_value2($dess, $_POST['4_'.$i], "cr", $def_code,$condition);
      }
    }
    ...
    if ($condition == 0) {
      $query = $this->db->query("
           ...
           WHERE  t.`cl`='" . $this->sd['cl'] . "'  AND t.`bc`='" . $this->sd['branch'] . "'  AND t.`trans_code`='85'  AND t.`trans_no` ='" .$_POST['id']. "' AND 
           a.`is_control_acc`='0'");
...
JOIN  m_account a ON a.code=s.account_id where nno='$id' ORDER BY auto_no";
  ...
  public function chk_is_update(){
    $sql="SELECT nno 
          ...
          WHERE cl='".$this->sd['cl']."' AND bc='".$this->sd['branch']."' AND nno='".$_POST['code']."' AND TYPE='1'";
    $query=$this->db->query($sql);

...
class r_credit_sales_summary_group extends CI_Model{
  ...
  public function PDF_report($RepTyp=""){
    ...
    JOIN(SELECT * FROM `t_credit_sales_sum` WHERE cl='".$this->sd['cl']."' AND bc='".$this->sd['branch']."' AND is_cancel=0  AND `ddate` BETWEEN '".$_POST['from']."' AND '".$_POST['to']."')a ON a.cus_id=t_credit_note.`acc_code`
    WHERE `acc_code` IN (SELECT acc_code FROM `m_default_account` WHERE CODE='SALES_DISCOUNT') AND t_credit_note.cl='".$this->sd['cl']."' AND t_credit_note.bc='".$this->sd['branch']."' 
    AND t_credit_note.`ddate` BETWEEN '".$_POST['from']."' AND '".$_POST['to']."' AND dis_type='5' AND t_credit_note.is_cancel=0";

...
class m_item_commission extends CI_Model {
    ...
    public function data_table(){
        ...
        $this->db->limit(10);
        $query = $this->db->get('m_item_commission');
        
        foreach($query->result() as $r){
            $but = "<img src='".base_url()."img/edit.gif' onclick='set_edit(\"".$r->nno."\")' title='Edit' />&nbsp;&nbsp;";
            if($this->user_permissions->is_delete('m_item_commission')){$but .= "<img src='".base_url()."img/delete.gif' onclick='set_delete(\"".$r->nno."\")' title='Delete' />";}
    ...
    public function save(){
        ...
        try {
            ...
            if($_POST['code_'] == "0" || $_POST['code_'] == ""){
                if($this->user_permissions->is_add('m_item_commission')){
                    unset($_POST['code_']);
                    if(count($a)){ $this->db->insert_batch("m_item_commission", $a );}
                ...
                if($this->user_permissions->is_edit('m_item_commission')){
                    $this->set_delete();
                    unset($_POST['code_']);
                    if(count($a)){ $this->db->insert_batch("m_item_commission", $a );}
                    echo $this->db->trans_commit();
    ...
    public function check_code(){
        ...
        $this->db->limit(1);  
        echo $this->db->get('m_item_commission')->num_rows;
    }
    ...
    public function load(){
        $this->db->select(array(
            'm_item.code',
            'm_item.description',
            'm_item_commission.nno',
            'm_item_commission.ddate',
            'm_item_commission.description',
            'm_item_commission.rate',
            'm_item_commission.amount',
            'm_item_commission.date_from',
            'm_item_commission.date_to',
            'm_item_commission.inactive'
        ));
        $this->db->from("m_item");
        $this->db->join("m_item_commission","m_item.code=m_item_commission.code");
        $this->db->where("nno", $_POST['nno']);
        $query = $this->db->get();
        $a['c'] = $query->result(); 
    ...
    public function delete(){
        ...
        set_error_handler('exceptionThrower'); 
        try { 
            if($this->user_permissions->is_delete('m_item_commission')){
                $this->db->where('nno', $_POST['nno']);
                $this->db->delete('m_item_commission');
                echo $this->db->trans_commit();
            }else{
    ...
    public function max_no(){
        ...
        return $this->db->get('m_item_commission')->first_row()->nno+1;
    }

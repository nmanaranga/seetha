...
class r_sales_target_supervisor extends CI_Model {
	...
	public function PDF_report(){
		...
		$this->db->where("auto_no",$_POST['employee']);
		...
		$sql12 = $this->db->query("SELECT 
			...
			AND mb.supervisor='".$_POST['employee']."'
			GROUP BY t.bc ,t.mmonth
			ORDER BY t.bc ASC , t.mmonth ASC ");


		$sql1x= $this->db->query("SELECT IFNULL( SUM(sd.`target`),0) AS tar , IFNULL( SUM(sd.`achivement`),0) AS arc 
			FROM `t_sales_target_det` sd 
			LEFT JOIN m_branch mb ON sd.bc=mb.bc
			WHERE sd.mmonth <'".$_POST['txtmMonth']."'
			AND mb.supervisor='".$_POST['employee']."'");

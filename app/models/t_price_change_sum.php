...
public function get_item_detail(){
  ...
  $sql="SELECT m_item.model,t_price_change_det.*,t_price_change_sum.ddate,u.`discription` as user
  FROM t_price_change_det 
  JOIN t_price_change_sum ON t_price_change_sum.nno=t_price_change_det.nno  
  JOIN s_users u ON u.cCode=t_price_change_sum.oc
  JOIN m_item ON m_item.code=t_price_change_det.item 
  WHERE item='$code' 
  AND  t_price_change_sum.ddate BETWEEN '$from' AND '$to' GROUP BY nno LIMIT 25 ";

...
///////
		...
		*/
			...
			if(!empty($cluster)){
				$ccl=" AND `t_item_movement`.`cl` = '$cluster'";
			}else{
				$ccl=" ";
			}

			if(!empty($branch)){
				$bbc=" AND `t_item_movement`.`bc` = '$branch'";
			}else{
				$bbc=" ";
			}

			if(!empty($store)){
				$sstore=" AND `t_item_movement`.`store_code` = '$store'";
			}else{
				$sstore=" ";
			}

			if(!empty($department)){
				$ddep=" AND `m`.`department` = '$department'";
			}else{
				$ddep=" ";
			}

			if(!empty($main_category)){
				$m_cat=" AND `m`.`main_category` = '$main_category'";
			}else{
				$m_cat=" ";
			}

			if(!empty($sub_category)){
				$s_cat=" AND `m`.`category` = '$sub_category'";
			}else{
				$s_cat=" ";
			}
			...
			if(!empty($unit)){
				$uunit=" AND `m`.`unit` = '$unit'";
			}else{
				$uunit=" ";
			}

			if(!empty($brand)){
				$bbrand=" AND `m`.`brand` = '$brand'";
			}else{
				$bbrand=" ";
			}

			if(!empty($supplier)){
				$sup=" AND `m`.`supplier` = '$supplier'";
			}else{
				$sup=" ";
			}

			if(isset($_POST['minus'])){
				$qty_chk='having qty<0';
			}else{
				$qty_chk='having qty>0';
			}

			if(isset($_POST['non_serial'])){
				$serial=' AND `m`.`serial_no` = 0';
			}else{
				$serial='';
			}
			...
			if($re_type=="1"){
				...
				FROM t_item_movement 
				WHERE (ddate <='".$to."' 
				AND `t_item_movement`.`cl` = '".$this->sd['cl']."'
				AND `t_item_movement`.`bc` = '".$this->sd['branch']."'
				$sstore
			) 

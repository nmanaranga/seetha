      ...
      try {
            ...
            if(isset($_POST['0_'.$x],$_POST['2_'.$x],$_POST['4_'.$x])){
              if($_POST['0_'.$x] != "" && $_POST['2_'.$x] != "" && $_POST['4_'.$x] != "" ){
                $t_grn_det[]= array(
                  ...
                  "foc"=>$_POST['3_'.$x],
                  "amount"=>$_POST['t_'.$x],
                  "batch_no"=>$this->utility->get_batch_no($_POST['0_'.$x],$_POST['4_'.$x],$_POST['max_'.$x],$_POST['min_'.$x])
                ); 
        ...
        if($_POST['hid'] == "0" || $_POST['hid'] == ""){
            ...
            if($account_update==1){
              ...
              //if(isset($t_item_batch)){if(count($t_item_batch)){  $this->db->insert_batch("t_item_batch", $t_item_batch);}}
        ...
        if($_POST['0_'.$x] != "" || !empty($_POST['0_' . $x])){
          if($this->check_is_serial_items($_POST['0_'.$x])==1){
            ...
            $p = explode("-",$pp[$t]);
              ...
              "date"=>$_POST['ddate'],
              "item"=>$_POST['0_'.$x],
              "batch"=>$this->utility->get_batch_no($_POST['0_'.$x],$_POST['4_'.$x],$_POST['max_'.$x],$_POST['min_'.$x]),
              "serial_no"=>$p[0],
              "other_no1"=>$p[1],
              "other_no2"=>$p[2],
              "cost"=>$_POST['4_'.$x],
   ...
   public function load_po_nos(){
    ...
    if($_POST['sup'] != ""){
       ...
       $sql="SELECT trans_no AS nno, SUM(request_qty),SUM(received_qty),p.ddate AS date,
              ...
              FROM t_po_trans
              JOIN (SELECT ps.ddate,ps.supplier,ps.total_amount,s.`name`,ps.`inactive`,ps.`nno`,ps.cl,ps.bc 
                FROM t_po_sum ps
                JOIN m_supplier s ON s.`code` = ps.`supplier`
                WHERE inactive='0' AND TYPE='".$_POST['type']."' AND cl ='$cl' AND bc='$bc' AND supplier = '$supplier') p
      ...
      $sql="SELECT trans_no AS nno, SUM(request_qty),SUM(received_qty),p.ddate AS date,
          ...
          JOIN (SELECT ps.ddate,ps.supplier,ps.total_amount,s.`name`,ps.`inactive`,ps.`nno`,ps.cl,ps.bc 
            FROM t_po_sum ps
            JOIN m_supplier s ON s.`code` = ps.`supplier`
            WHERE inactive='0' AND TYPE='".$_POST['type']."' AND cl ='$cl' AND bc='$bc') p
          ON p.`nno` = t_po_trans.`trans_no` AND p.cl=t_po_trans.`cl` AND p.bc = t_po_trans.bc
...
,                        t_item_movement_sub.`sub_item` 
      ...
      $this->db->select(array('t_grn_sum.oc', 's_users.discription','action_date'));
    ...
    function check_sub_item_with_batch_item($item_code){
      ...
      if($item!=""){
        $sql="SELECT * FROM `m_item_sub` WHERE item_code = '$item';";
        $query=$this->db->query($sql);
        if($query->num_rows()>0){
            $sql="SELECT batch_item FROM m_item WHERE code='$item'";

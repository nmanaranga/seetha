...
class t_internal_transfer_return extends CI_Model {
  ...
  public function base_details(){
    ...
    $a['sub_max_no']= $this->get_max_no_in_type_transfer("t_internal_transfer_return_sum","sub_no","branch");
  ...
  public function load_all_return(){
    $sql="SELECT s.nno as mainno,s.sub_no as nno,ddate,s.`to_bc`,s.`to_cl`,b.`name`,s.trns_mode
    ...
    $a .= "<th class='tb_head_th'>To BC</th>";
    ...
    foreach ($query->result() as $r) {
      ...
      $a .= "<td>" . $r->to_bc . "</td>";
      $a .= "<td>" . $r->name . "</td>";
      $a .= "<td style='display:none;'>" . $r->trns_mode . "</td>";
      $a .= "<td style='display:none;'>" . $r->mainno . "</td>";
      $a .= "<td style='display:none;'>" . $r->to_cl . "</td>";
      $a .= "</tr>";
    }
    $a .= "</table>";
    echo $a;
  ...
  public function load_return_details(){
    ...
    d.`qty`, 
    0 AS balance, 
    (b.`purchase_price` * d.qty) AS amount  
    FROM t_internal_transfer_sum s 
    JOIN t_internal_transfer_det d ON s.`cl` = d.`cl` AND s.`bc` = d.`bc` AND s.`nno` = d.`nno` AND s.`sub_no`=d.`sub_no`
    JOIN m_item m ON m.`code` = d.`item_code` 
    JOIN t_item_batch b ON b.`item` = d.`item_code` AND b.`batch_no`=d.`batch_no`
    WHERE s.cl = '".$this->sd['cl']."' 
    AND s.bc = '".$this->sd['branch']."' 
    AND s.type = '".$_POST['type']."' 
    AND s.`trans_code` = '42' 
  ...
  public function load_balance(){
    $sql="SELECT d.bc,s.`sub_no`,d.nno, s.`type`, r.bc, r.sub_no,  d.`item_code`, r.item_code AS receive_item, d.`qty` AS issue_qty, r.qty AS receive_qty, IFNULL(d.qty,0)-IFNULL(r.qty,0) AS balance  
    FROM `t_internal_transfer_sum` s
    JOIN `t_internal_transfer_det` d ON s.`cl`=d.`cl` AND s.`bc`=d.`bc` AND s.`nno`=d.`nno`
    LEFT JOIN (SELECT s.sub_no,s.cl,s.bc,s.`nno`,d.`item_code`,d.`qty`,s.`ref_trans_code`,s.`ref_trans_no`,s.`issue_bc`,s.`trans_code` FROM t_internal_transfer_sum s 
    JOIN t_internal_transfer_det d ON s.`cl`=d.`cl` AND s.`bc`=d.`bc` AND s.`nno`=d.`nno`
    WHERE s.trans_code='43' GROUP BY d.cl,d.bc,d.nno,d.`item_code`
    ) r ON s.`to_cl`=r.cl AND s.`to_bc`=r.bc AND s.nno=r.ref_trans_no AND r.trans_code='43' AND d.`item_code`=r.item_code
    WHERE s.bc='".$this->sd['branch']."' AND s.`sub_no`='".$_POST['no']."'
    AND s.`type`='".$_POST['type']."'
    GROUP BY d.cl,d.bc,d.nno,d.`item_code`";
      ...
      try { 
        ...
        if($validation_status==1){
         ...
         $data=array(
          ...
          "issue_no_main" => $_POST['issue_no_main'],
          "note" => $_POST['note'],
          "vehicle" => $_POST['vehicle'],
          "to_bc" => $_POST['br_code'],
          "tr_mode" => $_POST['tr_type'],
          "oc" => $this->sd['oc']
        );
         ...
         if($_POST['hid'] == "0" || $_POST['hid'] == ""){
          ...
          //  if($_POST['tr_type']=='0'){
           ...
           // }
            ...
            $sql="SELECT * FROM t_internal_transfer_sum
            WHERE cl='".$this->sd['cl']."' 
            AND bc='".$this->sd['branch']."' 
            AND type='".$_POST['types']."' 
            AND nno='".$_POST['issue_no_main']."' 
            AND trans_code='42'";
            $query=$this->db->query($sql);
            if($query->num_rows()>0){
              $t_cl=$query->first_row()->to_cl;
              $t_bc=$query->first_row()->to_bc;
              $nno=$query->first_row()->nno;
              $bal=$query->first_row()->net_amount;
            }else{
              $t_cl=$this->sd['cl'];
              $t_bc=$this->sd['branch'];
              $nno=$_POST['issue_no_main'];
              $bal=0;
            }  

            $balance=$bal-$_POST['total'];
            ...
            $t_credit_note = array(
              ...
              "post_date" => "",
              "is_cancel" => 0,
              "ref_trans_no" => $_POST['issue_no_main'],
              "ref_trans_code" => 80,
              "balance"=>$_POST['total']
            );
            ...
            if($_POST['tr_type']!=2){
              ...
              $this->load->model('trans_settlement');
              $this->trans_settlement->save_settlement("t_credit_note_trans", $br_acc_code, $_POST['ddate'], 17, $this->credit_max_no, 80, $_POST['issue_no_main'], $_POST['total'], "0");
            ...
            $this->db->where("bc", $this->sd['branch']);
            $this->db->where("type", $_POST['types']);
            $this->db->where("nno", $_POST['issue_no']);
            $this->db->update("t_internal_transfer_sum", array("balance"=>$balance)); 
          ...
          if($this->user_permissions->is_edit('t_internal_transfer_return')){
           ...
           //   }
            ...
            $this->trans_settlement->delete_internal_trans_sub_settlement('t_internal_transfer_trans',80,$this->max_no);  

            $sql="SELECT * FROM t_internal_transfer_sum
            WHERE cl='".$this->sd['cl']."' 
            AND bc='".$this->sd['branch']."' 
            AND type='".$_POST['types']."' 
            AND sub_no='".$_POST['issue_no']."' 
            AND trans_code='42'";
  ...
  public function account_update($condition) {
    ...
    $good_in_transit_acc   = $this->utility->get_default_acc('GOOD_TRANSIT');
      $revenue_int_sales_acc = $this->utility->get_default_acc('REV_INTERNAL_SALE_RE'); // change account request mr gihan 2018-09-07
      ...
      $this->account->set_value2($des, $item_cost, "cr", $internal_cost_acc,$condition);
      $this->account->set_value2($des, $item_cost, "dr", $stock_acc,$condition);
      $this->account->set_value2($des, $item_cost, "cr", $good_in_transit_acc,$condition);
      $this->account->set_value2($des, $item_cost, "dr", $revenue_int_sales_acc,$condition);
...
public function get_display() {
  ...
  LEFT JOIN m_branch mb ON mb.bc=s.to_bc
  LEFT JOIN m_vehicle_setup v ON v.`code` = s.`vehicle` AND v.`cl` = s.`cl` AND s.`bc` = v.`bc`
...
public function PDF_report() {
  ...
  JOIN m_stores m ON m.`code` = s.`from_store` AND m.`cl` = s.`cl` AND m.`bc` = s.`bc`  
  JOIN m_stores mm ON mm.`code` = s.`to_store` AND mm.`cl` = s.`cl` AND mm.`bc` = s.`bc`
  JOIN m_vehicle_setup v ON v.`code` = s.`vehicle`
  LEFT JOIN m_branch mb ON mb.bc=s.to_bc
  ...
  and `t_internal_transfer_return_det`.`accept_qty`>0
...
public function delete(){
  ...
  try {
    ...
    if($this->user_permissions->is_delete('t_internal_transfer_return')){
      ...
      $this->trans_settlement->delete_internal_trans_sub_settlement('t_internal_transfer_trans',80,$trans_no);

    ...
    else
    {
      ...
      for ($x = 0; $x < 25; $x++) {
        ...
        public function load(){
         ...
         $this->db->join('r_groups','r_groups.code=t_unloading_sum.group_sale_id');
...
/*        $sql="SELECT m_item.code as item, 
        m_item.`description`,
        m_item.`model`,
        m_item.`warranty`,
        ...
        AND group_sale='$_POST[group_sale_id]' 
...
/*
$sql="SELECT 
qry_current_stock_group.item,
qry_current_stock_group.`description`,
qry_current_stock_group.`model`,
t_item_batch.`max_price`,
t_item_batch.`batch_no`,
t_item_batch.`min_price`,
t_item_batch.purchase_price,
...
AND qry_current_stock_group.cl='$cl' 
AND qry_current_stock_group.bc='$bc' 
AND (qry_current_stock_group.`description` LIKE '%$_POST[search]%' 
OR qry_current_stock_group.`item` LIKE '%$_POST[search]%' 
OR qry_current_stock_group.model LIKE '%$_POST[search]%' 
) 
...
public function PDF_report(){
  ...
  $sql="SELECT s.cl,s.bc,d.code as item, d.batch_no,m.description ,d.qty as dqty,s.ref_no,s.store_to,
                    s.store_from,s.memo,s.action_date,s.group_sale_id,d.min,d.max,m.model
            FROM t_unloading_sum s 
            JOIN t_unloading_det d ON s.cl=d.cl and s.bc=d.bc and s.nno=d.nno 
...
public function loading_details(){
  $sql="SELECT s.`group_sale_id`,g.`name` 
        FROM t_loading_sum s
        JOIN r_groups g ON s.`cl`=g.`cl` AND s.`bc`=g.`bc` AND s.`group_sale_id`=g.`code`
        WHERE s.cl='".$this->sd['cl']."'
        AND s.bc='".$this->sd['branch']."'
        AND s.`nno`='".$_POST['id']."'";
  $a['sum']=$this->db->query($sql)->result();
  ...
  $sql="SELECT d.`code`,
          ...
          d.`max`,
          q.`qty`
        FROM `t_loading_det` d
        JOIN m_item m ON m.`code`=d.`code`
        JOIN qry_current_stock q ON q.cl=d.cl AND q.bc=d.bc AND q.`item`=d.`code` AND q.`batch_no`=d.`batch_no`

...
class r_cus_balance_details extends CI_Model {
	...
	public function PDF_report($RepTyp="")
	{
		...
		$sql.=" GROUP BY s.cl, s.bc, s.nno HAVING SUM(dr)-SUM(cr) > 0   
		UNION ALL 
		...
		INNER JOIN (SELECT cus_id, ddate, nno, net_amount, is_install FROM t_credit_sales_sum WHERE is_cancel='0'";
		if(!empty($cluster))
		{
			$sql.=" AND cl='$cluster' ";
		}

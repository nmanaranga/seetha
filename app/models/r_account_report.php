...
class r_account_report extends CI_Model {
    ...
    public function PDF_report($RepTyp=""){
        ...
        $sqll="SELECT t_account_trans.*,
        IFNULL(cd.`card_no`,0)AS card_no ,
        t_trans_code.`description` AS det,
        t_account_trans.description AS description, 
        t_trans_code.`code` as t_code,
        IF(t_trans_code.`code`='43',CONCAT(t_account_trans.ref_cl,t_account_trans.`ref_bc`,LPAD(t_account_trans.trans_no, 6, 0)),CONCAT(t_account_trans.cl,t_account_trans.`bc`,LPAD(t_account_trans.trans_no, 6, 0))) AS tno
        FROM t_account_trans 
        LEFT JOIN t_trans_code ON t_trans_code.`code`= t_account_trans.`trans_code` 
        LEFT JOIN (SELECT cl,bc,trans_code,trans_no,SUM(amount) AS amount,card_no FROM `opt_credit_card_det` WHERE bc='$branch') cd 
        ON cd.`cl` = t_account_trans.`cl` 
        AND cd.bc = t_account_trans.`bc` 
        AND cd.`amount` = t_account_trans.`dr_amount` 
        AND cd.`trans_no` = t_account_trans.`trans_no`
        WHERE ddate!='' 
        AND ddate BETWEEN '$_POST[from]' AND '$_POST[to]'
        ";
        if(!empty($cluster)){
            $sqll.=" AND t_account_trans.`cl` = '$cluster'";
        }
        if(!empty($branch)){
            $sqll.=" AND t_account_trans.`bc` = '$branch'";
        }  
        if($_POST['acc_code']!=""){
            $sqll.=" AND t_account_trans.acc_code='$_POST[acc_code]'";
        }  
        //$sqll.=" GROUP BY card_no";
       // $sqll.=" ORDER BY ddate,auto_no";
        $sqll.=" ORDER BY ddate,action_date";

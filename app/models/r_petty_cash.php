  ...
  public function PDF_report(){
    ...
    $sql="SELECT 
      ...
      $sql1="SELECT 
         ...
         if(!empty($_POST['from']))
        {
           $sql1.=" AND PCM.date between '".$_POST['from']."' AND '".$_POST['to']."'";
        }
       if(!empty($_POST['acc_code']))
        {
        ...
        if(!empty($cluster))
        {
           $sql1.=" AND PCM.cl = '$cluster'";
        }
       if(!empty($branch))
        {
            $sql1.=" AND PCM.bc = '$branch'";
        }
         
        if(!empty($_POST['t_range_from']) && empty($_POST['t_range_to']))
        {
           $sql1.=" AND PCM.`no` >= '$no_range_frm'";
        }
        
         if(!empty($_POST['t_range_to']) && empty($_POST['t_range_from']))
        {
           $sql1.=" AND PCM.`no` <= '$no_range_to'";
        }

        if(!empty($_POST['t_range_to']) && !empty($_POST['t_range_to']))
        {
           $sql1.=" AND PCM.`no` BETWEEN  '$no_range_frm' AND '$no_range_to'";
        }

        $sql1.="ORDER BY PCM.cl,PCM.bc,PCM.no ASC";
